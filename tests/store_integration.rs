...
fn test_store_save_and_load() {
    ...
    assert_eq!(
        loaded_store.settings.theme.as_ref().unwrap().name,
        "test-theme"
    );
    assert_eq!(
        loaded_store.settings.theme.as_ref().unwrap().source,
        "test-source"
    );
}
...
fn test_store_load_nonexistent_creates_default() {
    ...
    assert!(
        store.settings.theme.is_none(),
        "Default settings should have no theme"
    );
}
