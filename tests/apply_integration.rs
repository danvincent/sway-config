...
fn test_render_waybar_config_json() {
    ...
    let json: serde_json::Value =
        serde_json::from_str(&waybar.config_json).expect("Waybar config JSON must be valid");
...
fn test_render_autostart_entries() {
    ...
    let entry1 = model
        .autostart
        .iter()
        .find(|e| e.exec_line.contains("example-daemon"));
...
fn test_apply_writes_empty_idle_conf_when_disabled() {
    ...
    let idle_conf_path = tempdir.path().join("sway/config.d/idle.conf");
    assert!(
        idle_conf_path.exists(),
        "idle.conf should exist even when disabled"
    );

    let content = fs::read_to_string(&idle_conf_path).expect("Failed to read idle.conf");
    assert!(
        content.is_empty() || content.trim().is_empty(),
        "idle.conf should be empty when disabled"
    );
}
...
fn test_apply_writes_empty_autostart_conf_when_all_disabled() {
    ...
    assert!(
        content.is_empty() || content.trim().is_empty(),
        "autostart.conf should be empty when all entries are disabled"
    );
}
...
fn test_apply_writes_empty_waybar_config_json_when_disabled() {
    ...
    assert_eq!(
        content.trim(),
        "{}",
        "config.jsonc should be empty JSON object when waybar is disabled"
    );
}
...
fn test_apply_clears_stale_files() {
    ...
    let idle_content_before =
        fs::read_to_string(&idle_conf_path).expect("Failed to read idle.conf");
    assert!(
        !idle_content_before.is_empty(),
        "idle.conf should have content initially"
    );
    ...
    assert!(
        idle_content_after.is_empty() || idle_content_after.trim().is_empty(),
        "idle.conf should be empty after disabling"
    );

    assert!(
        autostart_conf_path.exists(),
        "autostart.conf should still exist"
    );
    let autostart_content_after =
        fs::read_to_string(&autostart_conf_path).expect("Failed to read autostart.conf");
    assert!(
        autostart_content_after.is_empty() || autostart_content_after.trim().is_empty(),
        "autostart.conf should be empty after disabling"
    );

    assert!(
        waybar_config_path.exists(),
        "config.jsonc should still exist"
    );
    let waybar_content_after =
        fs::read_to_string(&waybar_config_path).expect("Failed to read config.jsonc");
    assert_eq!(
        waybar_content_after.trim(),
        "{}",
        "config.jsonc should be empty JSON object after disabling"
    );
}
